...
GOOGLE_FR_VOICE_FEMALE = 'Chirp3-HD-Zephyr'
...
def google_cloud_tts(s, filename, language=LANGUAGE_FR, genre=GENRE_FEMININE, file_id=None, file_title=None, ftp_path=None, speaking_rate=None, ssml=False, verbs=False, overwrite=False):
    if not overwrite and check_if_file_exists(filename + '.mp3', ftp_path or get_path(language)):
...
# 	else:
# 		filename = filename+f'.{mp3_filename.split(".")[-1]}'
# 	if genre == GENRE_FEMININE and not 'fem' in mp3_filename and not verbs:
# 		if check_if_file_exists(filename, ftp_path or get_path(language_code=language_code, verbs=verbs)):
# 			delete_ftp_file(filename, ftp_path or get_path(language_code=language_code, verbs=verbs))
# 		return None
#
...
def get_audio_links(words_list:List[Tuple[int, str, str]], shtooka=True):
    result = []
