		...
		if self.is_valid():
			...
			return dict(
				...
				empty_header= 'Выберите уроки (можно одновременно выбирать'
				              ...
				              'слов». Или восстановите предыдущий выбор'
				              '(если он был) соответствующей кнопкой.',
				empty_body='''<p>После получения списка вы можете делать
				    ...
				    отмеченных слов сохраняются на лету — вы можете спокойно
				    уходить с этой страницы и при следующем заходе восстановить
				    ее состояние одним нажатием кнопки.</p>''',
			)
