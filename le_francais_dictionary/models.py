...
class Partner(models.Model):
    ...
    def to_dict(self):
        ...
        data['name'] = self.name
        data['packets'] = [
            p.to_dict for p in Packet.objects.filter(partner=self)
        ]

    def save(self, *args, **kwargs):
        if self.path is None or self.path == '':
            self.path = f"partners/{slugify(self.name, allow_unicode=True)}/"
...
class Word(models.Model):
    ...
    genre = models.CharField(choices=GENRE_CHOICES, max_length=10, null=True,
                             verbose_name='Gender', blank=True, validators=[])
    ...
    cross_site_available = models.BooleanField(default=False, blank=True)
    cross_site_id = models.PositiveIntegerField(blank=True, null=True)
    cross_site_site_name = models.CharField(max_length=10, null=True, blank=True)
    ...
    def to_dict(self, with_user=False, user=None):
        ...
        if self.cross_site_available:
            data['remote_id'] = self.cross_site_id
        return data
    ...
    def create_polly_task_v2(self):
        ...
        for voice_text in text.split('|'):
            new_task = PollyTask.objects.create(
                text=voice_text,
            ...
            if voice == VOICE_ID_REMI:
                voice = VOICE_ID_LEA
            else:
                voice = VOICE_ID_REMI

        if len(tasks) > 1:
            result_task = join_polly_tasks(
                *tasks,
                params=[{}] * len(tasks),
                text=text,
                tags=tags,
                save_path=f"{self.packet.partner.path}flashcards/words/" if self.packet.partner else None
            )
        else:
            result_task = tasks[0]
            result_task.save_to_file(
                save_path=f"{self.packet.partner.path}flashcards/words/" if self.packet.partner else None,
                tags=tags
            )
...
class WordTranslation(models.Model):
    ...
    translation_string = models.TextField(null=True, blank=True, default=None)
    ...
    @property
    def table_html(self):
        if self.word.part_of_speech in [PARTOFSPEECH_ADJECTIVE, PARTOFSPEECH_NOUN] and (
                self.word.genre == GENRE_BOTH or self.word.genre == GENRE_EPICENE):
            return re.sub(
                ADJECTIVE_GENRE_PARENTHESIS_PATTERN,
                r'<span class="genre-cc-enabled genre-feminine pos-cc-enabled pos-adj">\1</span>',
                self.translation
        ...
        if len(tasks) > 1:
            result_task = create_joined_task_v2(
                *tasks,
                ...
                text=text,
                tags=tags,
                save_path=f"{self.word.packet.partner.path}flashcards/translations/" if self.word.packet.partner else None
            )
        else:
            result_task = tasks[0]
            result_task.save_to_file(
...
class UserWordRepetition(models.Model):
    ...
    def update_repetition(self, save):
        ...
        datetime = timezone.make_aware(
            timezone.make_naive(datetime, self.user.pytz_timezone).replace(hour=0, minute=0, second=0, microsecond=0),
            self.user.pytz_timezone)
