...
[]);var a=$(this).find(":selected").map(function(){return $(this).text()}).get().join("|");""===a?dt.api().column(-1).search(a,!1,!0).draw():(a="^"+a+"$",dt.api().column(-1).search(a,!0,!1).draw());add_selected_filtered_alert(dt)});$selectStars=$("#starFilter")});showDeleted($("#showDeleted")[0].checked);$("#showDeleted").on("change",function(a){a.preventDefault();saveFilters("afterLoad",[this.id],["checked"]);showDeleted(this.checked);add_selected_filtered_alert(dt)});var c=$(".play");c.unbind();
c.click(function(a){a=$(a.target).data("audiosrc");voice(a)});b.on("user-select.dt.dtCheckboxes",function(a,b,c,d,e){$(e.target).hasClass("play")&&a.preventDefault()});b.on("select.dt.dtCheckboxes deselect.dt.dtCheckboxes",function(a,b,c,d,e){if(savingFiltersEnabled){tableFilters.selectAfterFilter={};var f=[];$.each(get_selected_filtered(dt),function(a,b){f.push("checkbox-select-"+b)});saveFilters("selectAfterFilter",f,["checked"],"on",!0)}});void 0!==a&&a()}})},error:errorLoading,beforeSend:tableReloading});
