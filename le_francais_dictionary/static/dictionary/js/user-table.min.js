...
""===a?dt.api().column(-1).search(a,!1,!0).draw():(a="^"+a+"$",dt.api().column(-1).search(a,!0,!1).draw())});$selectStars=$("#starFilter")})}})}});$(".undertable").show()}function get_selected(a){a=a.api().column(0).checkboxes.selected();var b=[];$.each(a,function(a,d){b.push(d)});return b}
$(document).ready(function(){$("#filterWordsForm").submit(function(a){a.preventDefault();updateTable()});$("#startApp").on("click",function(){$.ajax(Urls["dictionary:standalone"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),success:function(a){window.location.href=Urls["dictionary:standalone"]()}})});$("#markWords").on("click",function(){dt.api().column(0).checkboxes.selected();$.ajax(Urls["dictionary:mark_words"](),
{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),success:function(a){updateTable()}})});$("#unmarkWords").on("click",function(){dt.api().column(0).checkboxes.selected();$.ajax(Urls["dictionary:unmark_words"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),success:function(a){updateTable()}})});$("#main-frm").on("submit",function(a){var b=
this,c=dt.api().column(0).checkboxes.selected();$.each(c,function(a,c){$(b).append($("<input>").attr("type","hidden").attr("name","id[]").val(c))});a.preventDefault()})});
