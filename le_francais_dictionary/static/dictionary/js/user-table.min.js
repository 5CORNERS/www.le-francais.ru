...
function showDeleted(a){a?dt.api().column(2).search("^True|False$",!0,!1).draw():dt.api().column(2).search("^False$",!0,!1).draw()}function emptyTable(a){$table.find("tbody").empty().html('<tr><td colspan="5" style="text-align:center">'+a+"</td></tr>")}function errorLoading(){emptyTable('<div class="row"><div class="col-12"><h5 style="margin:1rem">\u041e\u0448\u0438\u0431\u043a\u0430 :(</h5></div> </div>')}
...
$(document).ready(function(){$("#filterWordsForm").submit(function(a){a.preventDefault();updateTable()});$("#startApp").on("click",function(){$.ajax(Urls["dictionary:standalone"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),error:errorLoading,success:function(a){window.location.href=Urls["dictionary:standalone"]()},beforeSend:emptyTable("")})});$("#markWords").on("click",function(){dt.api().column(0).checkboxes.selected();
$.ajax(Urls["dictionary:mark_words"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),success:function(a){updateTable()},error:errorLoading,beforeSend:tableReloading})});$("#unmarkWords").on("click",function(){dt.api().column(0).checkboxes.selected();$.ajax(Urls["dictionary:unmark_words"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),
success:function(a){updateTable()},error:errorLoading,beforeSend:tableReloading})});$("#main-frm").on("submit",function(a){var b=this,c=dt.api().column(0).checkboxes.selected();$.each(c,function(a,c){$(b).append($("<input>").attr("type","hidden").attr("name","id[]").val(c))});a.preventDefault()})});
