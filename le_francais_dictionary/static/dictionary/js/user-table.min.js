...
$(document).ready(function(){$("#filterWordsForm").submit(function(a){a.preventDefault();updateTable()});$("#startApp").on("click",function(){ids=get_selected(dt);0===ids.length?($("#startApp").popover({content:"\u0412\u044b \u043d\u0435 \u0432\u044b\u0431\u0440\u0430\u043b\u0438 \u043d\u0438 \u043e\u0434\u043d\u043e\u0433\u043e \u0441\u043b\u043e\u0432\u0430",placement:"top",trigger:"manual"}).popover("show"),setTimeout(function(){$("#startApp").popover("hide").popover("dispose")},2E3)):($("#startApp").popover("dispose"),
$.ajax(Urls["dictionary:app"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),error:errorLoading,success:function(a){window.location.href=Urls["dictionary:app"]()},beforeSend:emptyTable("")}))});$("#markWords").on("click",function(){dt.api().column(0).checkboxes.selected();$.ajax(Urls["dictionary:mark_words"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),
csrfmiddlewaretoken:csrf}),success:function(a){updateTable()},error:errorLoading,beforeSend:tableReloading})});$("#unmarkWords").on("click",function(){dt.api().column(0).checkboxes.selected();$.ajax(Urls["dictionary:unmark_words"](),{type:"POST",contentType:"application/json",dataType:"json",data:JSON.stringify({words:get_selected(dt),csrfmiddlewaretoken:csrf}),success:function(a){updateTable()},error:errorLoading,beforeSend:tableReloading})});$("#main-frm").on("submit",function(a){var b=this,c=dt.api().column(0).checkboxes.selected();
$.each(c,function(a,c){$(b).append($("<input>").attr("type","hidden").attr("name","id[]").val(c))});a.preventDefault()})});
