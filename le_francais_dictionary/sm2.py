    ...
    def sm2_next_repetition_date(self) -> None:
        ...
        for i, data in enumerate(self.dataset):
                ...
                if n == 0:
                    ...
                    if answer_datetime > current_repetition_date:
                        ...
                        if n == 1 or n == 2:
                            ...
                            last_repetition_delta = repetitions_datetimes[-2] - \
                                                    repetitions_datetimes[-3]
                            # новая пересчитанная дата повторения
                            new_current_repetition_date = repetition_datetime_to_date(
                                last_repetition_date + timedelta(
                                    days=last_repetition_delta.days * new_e_factor),
                                answer_timezone)
            ...
            if new_repetition_date is not None:
                repetitions_datetimes.append(
                    repetition_datetime_to_date(new_repetition_date,
                                                answer_timezone))
        ...
        if qualities_list:
            ...
            self.mean_quality = sum(qualities_list) / len(qualities_list)
        else:
            self.last_quality = 0
            self.mean_quality = 0
...
def sm2_response_quality(data, zeros_dataset, repetition_time=None):
    ...
    if mistakes < 0:
        mistakes = 0
    delay = data.delay
    if data.custom_grade is not None and isinstance(data.custom_grade,
                                                    int) and 0 <= data.custom_grade <= 5:
        q = data.custom_grade
        ...
        for delay_start, delay_end, delay_minus_start, delay_minus_end in ranges:
            if delay_start <= delay <= delay_end:
                ...
                q += delay_minus_start + (
                            (delay_minus_end - delay_minus_start) * delay_ratio)
                break
