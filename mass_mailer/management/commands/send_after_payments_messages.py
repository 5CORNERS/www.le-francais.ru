...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '-td', '--timedelta',
            default='0:0:15:0', dest='timedelta', type=self.parse_timedelta
        )

    def parse_timedelta(self, time_str: str):
        try:
            days, hours, minutes, seconds = map(int, time_str.split(':'))
            return datetime.timedelta(days=days, hours=hours, minutes=minutes, seconds=seconds)
        except ValueError:
            raise CommandError("Timedelta must be in the format 'D:H:M:S'")
    ...
    def handle(self, *args, **options):
        ...
        send_message(message, filter_users_with_payments_with_activations(message, options['timedelta']))


def filter_users_with_payments_with_activations(message, timedelta):
    ...
    users = User.objects.filter(
        ...
        pk__in=[log.recipient_id for log in
                MessageLog.objects.filter(result__in=MessageLog.RESULTS_SUCCESS, message=message)]
    ).exclude(
        pk__in=[p.user_id for p in Profile.objects.filter(
            subscribed=False, user__isnull=False
        )]
    )

    user_lessons = UserLesson.objects.select_related('lesson').filter(user_id__in=[u.pk for u in users]).order_by(
        'date')
