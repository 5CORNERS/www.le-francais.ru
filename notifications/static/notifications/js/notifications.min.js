...
function updateBadge(){$.ajax({url:Urls["notifications:get_new_count"](),success:function(a){0<a?($("#notify-badge-nav").html(a).show(),notificationsUpdateDatetime=Date.now()):$("#notify-badge-nav").html(a).hide()}})}$(document).ready(function(){$(".notify-dropdown .nav-link").on("click",get_drop_content);var a=setIntervalRun(6E4,ifHasNewNotification,updateBadge);document.addEventListener("visibilitychange",function(){"visible"===document.visibilityState?a.start():a.stop()})});
function check_notifications(a){0<a.length&&($("#notify-badge-nav").hide().html("0"),$.ajax(Urls["notifications:check_list"](),{type:"POST",data:{pks:a.toString()}}))}
