...
P.S. И да, вот же еще что! Я, кажется, нигде это не обговаривал: оплачивать можно карточкамиами, выпущенными в любой валюте и из любой страны — сумма пересчитывается по курсу в валюту карты автоматически. И это абсолютно безопасно — реквизиты карты вводятся на сайте банка, с которым устанавливается защищенное шифрованное соединение. И платеж подтверждается через SMS на Ваш телефон. Если все же оплата картой по каким-то причинам вызывает затруднения, есть альтернативные способы. Не стесняйтесь, пишите!
	...
	def handle(self, *args, **options):
		for u in find_users():
			...
			message = EmailMessage(
				to=[u.email],
				subject='Скидка на активацию уроков на le-francais.ru',
				body=MESSAGE.format(lookup(u)),
				from_email='ILYA DUMOV <ilia.dumov@files.le-francais.ru>',
