...
def page_advert_body(context, placement, page_type, gpt_disabled=None):
    try:
        snippet = PageLayoutAdvertisementSnippet.objects.filter(placement=placement, page_type=page_type).exclude(
            live=False).first()
        if gpt_disabled is None:
            gpt_disabled = context.get('is_gpt_disabled')
        return dict(
            body=snippet.body,
            placement=placement,
            snippet=snippet,
            gpt_disabled=gpt_disabled,
            request=context.get('request'),
