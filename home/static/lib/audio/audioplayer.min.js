...
typeof h!==typeof f&&(J?d.find("."+a.playPause).attr("title",g.strPause).find("a").html(g.strPause):h.find("."+a.playPause).attr("title",g.strPlay).find("a").html(g.strPause),h.addClass(a.playing));d.addClass(a.playing);t?c.play():c.Play()}return!1}function w(){!P||userlesson!==f&&!1!==userlesson||!1!==NEED_PAYMENT||(e.hidden&&d.hasClass(a.playing)?(r(!1),A=c.currentTime,c.src="",c.load(),C=!0):e.hidden||d.hasClass(a.playing)||(c.src=u,c.load(),c.currentTime=A,C&&(dispatchEvent(new Event("unhiddenWasPlaying")),
C=!1)))}if("audio"!=b(this).prop("tagName").toLowerCase())return!1;var n=b(this),u=n.attr("src"),x=n.get(0).getAttribute("autoplay"),p=n.get(0).getAttribute("loop"),t=!1,A=n.get(0).getAttribute("set-time"),D=!0===n.data("strict"),P=!0===n.data("forbid-background");x=""===x||"autoplay"===x;p=""===p||"loop"===p;"undefined"===typeof u?n.find("source").each(function(){u=b(this).attr("src");if("undefined"!==typeof u&&I(u))return t=!0,!1}):I(u)&&(t=!0);var d=b('<div class="'+g.classPrefix+'">'+(t?b("<div>").append(n.eq(0).clone()).html():
'<embed src="'+u+'" width="0" height="0" volume="100" autostart="'+x.toString()+'" loop="'+p.toString()+'" />')+'<div class="'+a.playPause+'" title="'+g.strPlay+'"><a href="#">'+g.strPlay+"</a></div></div>"),c=t?d.find("audio"):d.find("embed");c=c.get(0);var l=b(c).attr("number"),C;if(typeof l!==typeof f&&!1!==l)var h=d.clone();if(t){d.find("audio").css({width:0,height:0,visibility:"hidden"});d.append('<div class="'+a.time+" "+a.timeCurrent+'"></div><div class="'+a.bar+'"><div class="'+a.barLoaded+
'"></div><div class="'+a.barPlayed+'"></div></div><div class="'+a.time+" "+a.timeDuration+'"></div><div class="'+a.volume+'"><div class="'+a.volumeButton+'" title="'+g.strVolume+'"><a href="#">'+g.strVolume+'</a></div><div class="'+a.volumeAdjust+'"><div><div></div></div></div></div>');!0===b(c).data("downloadable")&&(p=b(c).find("source").attr("src"),D?d.append('<div class="'+a.download+'"><a download="true" href="#" class="'+a.downloadButton+' fa fa-download"></a></div>'):d.append('<div class="'+
a.download+'"><a download="true" href="'+p+'&download=true" class="'+a.downloadButton+' fa fa-download"></a></div><div class="audioplayer-space"></div>'),b(c).attr("id","lesson-audio"),b(c).attr("number",l));m.dispatchEvent(new CustomEvent("downloadButtonReady"));var y=d.find("."+a.bar),Q=d.find("."+a.barPlayed);d.find("."+a.barLoaded);var K=d.find("."+a.timeCurrent),L=d.find("."+a.timeDuration);p=d.find("."+a.volumeButton);var q=d.find("."+a.volumeAdjust+" > div"),R=d.find("."+a.downloadButton),
E=0,M=function(a){theRealEvent=k?a.originalEvent.touches[0]:a;c.currentTime=Math.round(c.duration*(theRealEvent.pageX-y.offset().left)/y.width())},N=function(a){theRealEvent=k?a.originalEvent.touches[0]:a;c.volume=Math.abs((theRealEvent.pageY-(q.offset().top+q.height()))/q.height())},S=c.volume,T=c.volume=.111;Math.round(1E3*c.volume)/1E3==T?c.volume=S:d.addClass(a.noVolume);L.html("&hellip;");K.text(B(0));c.addEventListener("loadeddata",function(){typeof l!==typeof f&&!1!==l&&(A?c.currentTime=A:
typeof localStorage["lecon-"+l]!==typeof f&&"undefined"!==localStorage["lecon-"+l]&&(c.currentTime=+localStorage["lecon-"+l]));L.text(B(c.duration));q.find("div").height(100*c.volume+"%");E=c.volume;event=new CustomEvent("lessonPlayerReady");m.dispatchEvent(event)});c.addEventListener("timeupdate",function(){K.text(B(c.currentTime));Q.width(c.currentTime/c.duration*100+"%");var a=b(c).attr("number");typeof a!==typeof f&&!1!==a&&0!==c.currentTime&&(localStorage["lecon-"+a]=c.currentTime)});c.addEventListener("volumechange",
