...
b.bar+'"><div class="'+b.barLoaded+'"></div><div class="'+b.barPlayed+'"></div></div><div class="'+b.time+" "+b.timeDuration+'"></div><div class="'+b.volume+'"><div class="'+b.volumeButton+'" title="'+e.strVolume+'"><a href="#">'+e.strVolume+'</a></div><div class="'+b.volumeAdjust+'"><div><div></div></div></div></div>');m=a(c).attr("data-downloadable");typeof m!==typeof g&&!1!==m&&"empty"!==m?(m=a(c).find("source").attr("src"),d.append('<div class="'+b.download+'"><a download="true" href="'+m+'&download=true" class="'+
b.downloadButton+' fa fa-download"></a></div><div class="audioplayer-space"></div>'),a(c).attr("id","lesson-audio"),a(c).attr("number",n)):"empty"===m&&d.append('<div class="'+b.download+'"><a download="true" class="'+b.downloadButton+' fa fa-download"></a></div>');event=new CustomEvent("downloadButtonReady");k.dispatchEvent(event);var x=d.find("."+b.bar),I=d.find("."+b.barPlayed);d.find("."+b.barLoaded);var E=d.find("."+b.timeCurrent),F=d.find("."+b.timeDuration);m=d.find("."+b.volumeButton);var p=
d.find("."+b.volumeAdjust+" > div"),A=0,G=function(a){theRealEvent=h?a.originalEvent.touches[0]:a;c.currentTime=Math.round(c.duration*(theRealEvent.pageX-x.offset().left)/x.width())},H=function(a){theRealEvent=h?a.originalEvent.touches[0]:a;c.volume=Math.abs((theRealEvent.pageY-(p.offset().top+p.height()))/p.height())},J=c.volume,K=c.volume=.111;Math.round(1E3*c.volume)/1E3==K?c.volume=J:d.addClass(b.noVolume);F.html("&hellip;");E.text(z(0));c.addEventListener("loadeddata",function(){typeof n!==typeof g&&
!1!==n&&(t?c.currentTime=t:typeof localStorage["lecon-"+n]!==typeof g&&"undefined"!==localStorage["lecon-"+n]&&(c.currentTime=+localStorage["lecon-"+n]));F.text(z(c.duration));p.find("div").height(100*c.volume+"%");A=c.volume;event=new CustomEvent("lessonPlayerReady");k.dispatchEvent(event)});c.addEventListener("timeupdate",function(){E.text(z(c.currentTime));I.width(c.currentTime/c.duration*100+"%");var b=a(c).attr("number");typeof b!==typeof g&&!1!==b&&0!==c.currentTime&&(localStorage["lecon-"+
b]=c.currentTime)});c.addEventListener("volumechange",function(){p.find("div").height(100*c.volume+"%");0<c.volume&&d.hasClass(b.mute)&&d.removeClass(b.mute);0>=c.volume&&!d.hasClass(b.mute)&&d.addClass(b.mute)});c.addEventListener("ended",function(){d.removeClass(b.playing);var e=a(c).attr("number");typeof e!==typeof g&&!1!==e&&(localStorage.removeItem("lecon-"+e),l.removeClass(b.playing))});x.on(v,function(a){G(a);x.on(y,function(a){G(a)})}).on(B,function(){x.off(y)});m.on("click",function(){d.hasClass(b.mute)?
(d.removeClass(b.mute),c.volume=A):(d.addClass(b.mute),A=c.volume,c.volume=0);return!1});p.on(v,function(a){H(a);p.on(y,function(a){H(a)})}).on(B,function(){p.off(y)})}else d.addClass(b.mini);typeof n!==typeof g&&!1!==n&&(typeof localStorage["lecon-"+n]!==typeof g&&"undefined"!==localStorage["lecon-"+n]&&(c.currentTime=+localStorage["lecon-"+n]),l.addClass(b.mini).addClass("pull-right"),l.attr("number",l.find("audio").attr("number")),l.find("audio").remove(),a("nav.navbar > .container > .d-flex.order-2 > .navbar-nav").prepend(l),
l.find("."+b.playPause).on("click",function(a){a.preventDefault();return f(!0)}));w&&d.addClass(b.playing);d.find("."+b.playPause).on("click",function(){return f(!1)});r.replaceWith(d)});return this}})(jQuery,window,document);
