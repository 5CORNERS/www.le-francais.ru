...
typeof h!==typeof f&&(J?d.find("."+a.playPause).attr("title",g.strPause).find("a").html(g.strPause):h.find("."+a.playPause).attr("title",g.strPlay).find("a").html(g.strPause),h.addClass(a.playing));d.addClass(a.playing);t?c.play():c.Play()}return!1}function w(){A&&(console.log("Visibility Changed. Playing: "+d.hasClass(a.playing)),e.hidden&&d.hasClass(a.playing)?(r(!1),B=c.currentTime,c.src="",c.load(),D=!0):e.hidden||d.hasClass(a.playing)||(c.src=u,c.load(),c.currentTime=B,D&&(dispatchEvent(new Event("unhiddenWasPlaying")),
